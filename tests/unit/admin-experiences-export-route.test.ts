...
import { GET as exportRoute } from "@/app/admin/imports/experiences/export/route";
import { GET as templateRoute } from "@/app/admin/imports/experiences/template/route";
...
describe("experiences export route", () => {
  ...
  it("returns an xlsx attachment for an admin", async () => {
    ...
    expect(response.headers.get("Content-Disposition")).toMatch(
      /^attachment; filename="sodoit-experiences-\d{4}-\d{2}-\d{2}\.xlsx"$/,
    );
    expect(listExportMock).toHaveBeenCalledOnce();
  });
});

describe("experiences template route", () => {
  it("returns 403 for an authenticated non-admin", async () => {
    requireAdminMock.mockResolvedValue({
      ok: false,
      error: "Admin access required.",
  ...
  it("returns the template xlsx for an admin without querying the database", async () => {
    ...
    expect(response.headers.get("Content-Disposition")).toBe(
      'attachment; filename="sodoit-experiences-template.xlsx"',
    );
    expect(listExportMock).not.toHaveBeenCalled();
  });
});
